...
if [[ -v OTEL_EXPORTER_OTLP_ENDPOINT ]] ; then
  echo "Enabling OpenTelemetry support"
  WSGI_CMD=("$VENVBIN/opentelemetry-instrument" "$VENVBIN/runwsgi")
  export ZOPE_INI_PATH="etc/zope-otel.ini"
else
  WSGI_CMD=("$VENVBIN/runwsgi")
  export ZOPE_INI_PATH="etc/zope.ini"
fi
